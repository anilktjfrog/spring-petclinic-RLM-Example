...
sleep 10
echo "\n\n**** Query by prop ****\n\n"
jf rt curl "/api/search/prop?repos=krishnam-mvn-dev-local&team=arch&ts=ts-${BUILD_ID}"

echo "\n\n**** Query by prop build.name ****\n\n"
jf rt curl "/api/search/prop?repos=krishnam-mvn-dev-local&build.name=${BUILD_NAME}"
